...
namespace easy-admin\Bundle\FrameworkBundle\DependencyInjection;

use Doctrine\Common\Annotations\Annotation;
use Doctrine\DBAL\Connection;
use Psr\Log\LogLevel;
use easy-admin\Bundle\FullStack;
use easy-admin\Component\Asset\Package;
use easy-admin\Component\Config\Definition\Builder\ArrayNodeDefinition;
use easy-admin\Component\Config\Definition\Builder\NodeBuilder;
use easy-admin\Component\Config\Definition\Builder\TreeBuilder;
use easy-admin\Component\Config\Definition\ConfigurationInterface;
use easy-admin\Component\Config\Definition\Exception\InvalidConfigurationException;
use easy-admin\Component\DependencyInjection\ContainerBuilder;
use easy-admin\Component\DependencyInjection\Exception\LogicException;
use easy-admin\Component\Form\Form;
use easy-admin\Component\HttpClient\HttpClient;
use easy-admin\Component\HttpFoundation\Cookie;
use easy-admin\Component\Lock\Lock;
use easy-admin\Component\Lock\Store\SemaphoreStore;
use easy-admin\Component\Mailer\Mailer;
use easy-admin\Component\Messenger\MessageBusInterface;
use easy-admin\Component\Notifier\Notifier;
use easy-admin\Component\PropertyAccess\PropertyAccessor;
use easy-admin\Component\PropertyInfo\PropertyInfoExtractorInterface;
use easy-admin\Component\RateLimiter\Policy\TokenBucketLimiter;
use easy-admin\Component\Serializer\Serializer;
use easy-admin\Component\Translation\Translator;
use easy-admin\Component\Uid\Factory\UuidFactory;
use easy-admin\Component\Validator\Validation;
use easy-admin\Component\WebLink\HttpHeaderSerializer;
use easy-admin\Component\Workflow\WorkflowEvents;
...
class Configuration implements ConfigurationInterface
{
    ...
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $willBeAvailable = static function (string $package, string $class, string $parentPackage = null) {
            ...
            $parentPackages[] = 'easy-admin/framework-bundle';
            ...
            ->children()
                ->arrayNode('secrets')
                    ...
                    ->children()
                        ...
                        ->scalarNode('decryption_env_var')->defaultValue('base64:default::easy-admin_DECRYPTION_SECRET')->end()
                    ->end()
                ->end()
            ->end()
        ;
        ...
        $rootNode
            ->children()
                ->arrayNode('form')
                    ...
                    ->{$enableIfStandalone('easy-admin/form', Form::class)}()
                    ->children()
                        ->arrayNode('csrf_protection')
                            ->treatFalseLike(['enabled' => false])
                            ->treatTrueLike(['enabled' => true])
                            ...
                            ->end()
                        ->end()
                        // to be deprecated in easy-admin 6.1
        ...
        $rootNode
            ->children()
                ->arrayNode('assets')
                    ...
                    ->{$enableIfStandalone('easy-admin/asset', Package::class)}()
        ...
        $rootNode
            ->children()
                ->arrayNode('translator')
                    ...
                    ->{$enableIfStandalone('easy-admin/translation', Translator::class)}()
        ...
        $rootNode
            ->children()
                ->arrayNode('validation')
                    ...
                    ->{$enableIfStandalone('easy-admin/validator', Validation::class)}()
                    ->children()
        ...
        $rootNode
            ->children()
                ->arrayNode('serializer')
                    ...
                    ->{$enableIfStandalone('easy-admin/serializer', Serializer::class)}()
                    ->children()
                        ->booleanNode('enable_annotations')->{!class_exists(FullStack::class) ? 'defaultTrue' : 'defaultFalse'}()->end()
        ...
        $rootNode
            ->children()
                ->arrayNode('property_access')
                    ...
                    ->{$willBeAvailable('easy-admin/property-access', PropertyAccessor::class) ? 'canBeDisabled' : 'canBeEnabled'}()
                    ->children()
        ...
        $rootNode
            ->children()
                ->arrayNode('property_info')
                    ...
                    ->{$enableIfStandalone('easy-admin/property-info', PropertyInfoExtractorInterface::class)}()
                ->end()
            ->end()
        ;
    }
                    ...
                    ->prototype('array')
                        ...
                        ->children()
                            ->scalarNode('log_level')
                                ->info('The level of log message. Null to let easy-admin decide.')
                                ->validate()
                            ...
                            ->scalarNode('status_code')
                                ->info('The status code of the response. Null to let easy-admin decide.')
                                ->validate()
        ...
        $rootNode
            ->children()
                ->arrayNode('lock')
                    ...
                    ->{$enableIfStandalone('easy-admin/lock', Lock::class)}()
                    ->beforeNormalization()
        ...
        $rootNode
            ->children()
                ->arrayNode('web_link')
                    ...
                    ->{$enableIfStandalone('easy-admin/weblink', HttpHeaderSerializer::class)}()
                ->end()
            ->end()
        ;
    }
        ...
        $rootNode
            ->children()
                ->arrayNode('messenger')
                    ...
                    ->{$enableIfStandalone('easy-admin/messenger', MessageBusInterface::class)}()
                    ...
                    ->children()
                        ...
                        ->arrayNode('serializer')
                            ...
                            ->children()
                                ...
                                ->end()
                                ->arrayNode('easy-admin_serializer')
                                    ->addDefaultsIfNotSet()
                                    ->children()
                                        ->scalarNode('format')->defaultValue('json')->info('Serialization format for the messenger.transport.easy-admin_serializer service (which is not the serializer used by default).')->end()
                                        ->arrayNode('context')
                                            ->normalizeKeys(false)
                                            ->useAttributeAsKey('name')
                                            ->defaultValue([])
                                            ->info('Context array for the messenger.transport.easy-admin_serializer service (which is not the serializer used by default).')
                                            ->prototype('variable')->end()
                                        ->end()
                                    ->end()
                                ->end()
        ...
        $rootNode
            ->children()
                ->arrayNode('http_client')
                    ...
                    ->{$enableIfStandalone('easy-admin/http-client', HttpClient::class)}()
                                ...
                                ->beforeNormalization()
                                    ...
                                    ->then(function ($config) {
                                        if (!class_exists(HttpClient::class)) {
                                            throw new LogicException('HttpClient support cannot be enabled as the component is not installed. Try running "composer require easy-admin/http-client".');
                                        }
        ...
        $rootNode
            ->children()
                ->arrayNode('mailer')
                    ...
                    ->{$enableIfStandalone('easy-admin/mailer', Mailer::class)}()
                    ->validate()
        ...
        $rootNode
            ->children()
                ->arrayNode('notifier')
                    ...
                    ->{$enableIfStandalone('easy-admin/notifier', Notifier::class)}()
        ...
        $rootNode
            ->children()
                ->arrayNode('rate_limiter')
                    ...
                    ->{$enableIfStandalone('easy-admin/rate-limiter', TokenBucketLimiter::class)}()
        ...
        $rootNode
            ->children()
                ->arrayNode('uid')
                    ...
                    ->{$enableIfStandalone('easy-admin/uid', UuidFactory::class)}()
                    ->addDefaultsIfNotSet()
                    ->children()
